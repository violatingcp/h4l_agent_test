...
> Read `methodology/02-phases.md` → "Phase 4" for full requirements.
> Read `methodology/04-output.md` for figure standards.
> Read `methodology/05-practices.md` for the blinding protocol.
...
`methodology/02-phases.md` → Phase 5 "Figure composition annotations."
...
1. **Full covariance chi2.** Every chi2 value in the artifact and AN uses
   ...
   `methodology/04-output.md` → Statistical methodology standards.
...
2. **Systematic variation sizing.** Every systematic variation is justified
   ...
   variations without documented motivation. See `methodology/02-phases.md`
...
3. **Extraction method hierarchy.** If a corrected differential distribution
   ...
   `methodology/02-phases.md` → Extraction Method Hierarchy.
...
5. **Per-systematic impact figures.** Every systematic source has a figure
   ...
   insufficient. See `methodology/04-output.md` → Systematic
   uncertainties.
...
6. **Resolving power statement.** After every result, state what deviations
   ...
   `methodology/04-output.md` → Interpretive quality standards.
...
- **REGRESS(N)** — fundamental issue traced to Phase N; follow the
  non-destructive regression protocol in `methodology/03-review.md` §6.6
...
- Phase requirements: `methodology/02-phases.md` → Phase 4
- Technique-specific requirements: `methodology/02-phases.md` → Phase 4 sub-phase descriptions
- Blinding: `methodology/05-practices.md`
- Review protocol: `methodology/03-review.md` → §6.2 (4-bot / 1-bot), §6.4
- Goodness-of-fit: `methodology/02-phases.md` → Phase 4 GoF requirements
...
`methodology/02-phases.md` → Phase 4 for full details.
...
commitments from being silently dropped. See `methodology/02-phases.md`
...
See `methodology/02-phases.md` → Phase 4a for the full specification.
...
See `methodology/02-phases.md` for the full specification.
...
Also check `methodology/03-review.md` for the full artifact and
...
See `methodology/03-review.md` for protocol. Write findings to
`review/{role}/` using session-named files (see
`methodology/06-appendix.md` for naming conventions).
