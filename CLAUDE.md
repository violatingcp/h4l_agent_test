...
Phase 4a: Expected results — executor + 1-bot+bib review
Phase 4b: 10% data validation — executor + 1-bot+bib review + human gate
Phase 4c: Full data results — executor + 1-bot review
Phase 5: Final documentation — executor + 2-bot review
```
...
requirements for every phase — especially 4b (1-bot+bib review AND human
gate, not just the human gate) and 4c (1-bot review, not unreviewed).
...
- **4a:** Statistical analysis — systematics, expected results. The executor
  performs the stats, writes AN v1 (ALL detail, expected results only), and
  typesets it (markdown → .tex → improve typesetting → compile PDF) in a
  single step. The review panel reads the compiled PDF — a review without a
  PDF is a process failure. PDF compilation mandatory before review.
- **4b:** 10% data validation. Compare to expected. The executor updates the
  stats and AN numbers to 10% data and recompiles the .tex + PDF. Human gate
  reviews the compiled PDF, not markdown.
- **4c:** Full data. Compare to **both** 10% and expected. The executor
  updates the stats and the AN with full results. PDF compilation
...
**Context splitting:** The executor normally handles stats, AN writing, and
typesetting in one step. Phase 4b and Phase 5 are context-intensive (AN
writing alongside statistical analysis), so when context pressure is high you
may split the executor's work into separate subagent invocations: one for
statistical analysis, another for AN writing/rendering. The AN-writing
subagent reads the inference artifact from disk.
...
| Topic | File | When |
|-------|------|------|
| Principles, inputs (incl. §2.3 numeric-constant citation), tools | `methodology/01-core.md` | Project start; coding phases |
| Phases 1–5 (§3), orchestration (§3a), artifacts (§5) | `methodology/02-phases.md` | Before each phase; writing artifacts |
| Review protocol + checklist (§6.2/§6.4/§6.5.1/§6.6/§6.7/§6.8) | `methodology/03-review.md` | Spawning reviewers; Phase 5 |
| Analysis note spec + plotting | `methodology/04-output.md` | Phase 4 (writing AN), figure phases, Phase 5 |
| Coding (§11/§11.4), downscoping (§12), blinding (§4), multichannel (§9) | `methodology/05-practices.md` | Coding phases; Phase 4; hitting limitations |
| Appendices: automation, prompts, sessions, deps, heuristics, integration | `methodology/06-appendix.md` | As referenced |
...
See `methodology/01-core.md` §2.3 for the full policy.
...
|-------|-------------------|-------------|
| 1 | `phase1_strategy/outputs/STRATEGY.md` | 2-bot |
| 2 | `phase2_exploration/outputs/EXPLORATION.md` | Self + plot validator |
| 3 | `phase3_selection/outputs/SELECTION.md` | 1-bot |
| 4a | `phase4_inference/4a_expected/outputs/INFERENCE_EXPECTED.md` + `ANALYSIS_NOTE_4a_v1.{md,tex,pdf}` | 1-bot+bib |
| 4b | `phase4_inference/4b_partial/outputs/INFERENCE_PARTIAL.md` + `ANALYSIS_NOTE_4b_v1.{md,tex,pdf}` | 1-bot+bib → human gate |
| 4c | `phase4_inference/4c_observed/outputs/INFERENCE_OBSERVED.md` + `ANALYSIS_NOTE_4c_v1.{md,tex,pdf}` | 1-bot |
| 5 | `phase5_documentation/outputs/ANALYSIS_NOTE_5_v{final}.{md,tex,pdf}` | 2-bot (incl. rendering + bib) |
...
See `methodology/03-review.md` for the full protocol. Key rules:
...
| Phase | Review type |
|-------|-------------|
| 1: Strategy | 2-bot (physics + arbiter) |
| 2: Exploration | Self-review + plot validator |
| 3: Processing | 1-bot (critical + plot validator) |
| 4a: Expected | 1-bot+bib (critical/arbiter + BibTeX validator; AN v1 has citations) |
| 4b: 10% validation | 1-bot+bib (critical/arbiter + BibTeX validator) → human gate |
| 4c: Full data | 1-bot |
| 5: Documentation | 2-bot (arbiter + rendering/BibTeX validator) |

**Iteration limits:** 2-bot: warn at 3, strong warn at 5, hard cap at 10. 1-bot: warn at 2, escalate after 3. All subagents use `model: "opus"`.
...
this triggers regression. See `methodology/03-review.md` §6.7 for the full protocol.
...
single regression iteration. See `methodology/03-review.md` §6.5.1.
...
- **REGRESS(N)** — fundamental issue traced to Phase N (see
  `methodology/03-review.md` §6.6 for the full non-destructive regression
  protocol)
...
Re-evaluate each downstream phase. After regression, the executor
...
See `methodology/03-review.md` §6.6 for the full protocol.
...
See `methodology/05-practices.md` for full coding practices.
...
See `methodology/04-output.md` for full plotting standards. Essentials:
...
Required AN sections — see `methodology/04-output.md` for the full
...
See `methodology/05-practices.md` for the full evaluation protocol.
